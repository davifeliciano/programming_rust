...
fn main() {
    ...
    if args.len() != 5 {
        ...
        eprintln!("Example: {} mandel.png 1000x750 -1.2,0.35 -1,0.20", args[0]);
        std::process::exit(1);
    }
...

    {
        ...
        crossbeam::scope(|spawner| {
            for (index, band) in bands.into_iter().enumerate() {
                ...
                let band_upper_left = pixel_to_complex(bounds, (0, top), upper_left, lower_right);
                let band_lower_right =
                    pixel_to_complex(bounds, (bounds.0, top + height), upper_left, lower_right);

                spawner.spawn(move |_| {
                    // move kw indicates that closure takes ownership of the variables it uses
...
/// return `None`.
fn escape_iters(c: Complex<f64>, iter_limit: usize) -> Option<usize> {
...
fn render(
    ...
    for row in 0..bounds.1 {
        for col in 0..bounds.0 {
            let point = pixel_to_complex(bounds, (col, row), upper_left, lower_right);
            pixels[row * bounds.0 + col] = match escape_iters(point, 255) {
                None => 0,
                Some(iters) => 255 - iters as u8,
            };
        }
    }
}
